#!/usr/bin/env python3
"""
Trust Score Update Script

This script updates the database schema to include trust_score_components
and calculates trust scores for all brokers using the new algorithm.
"""
...
def add_trust_score_components_column():
    """Add trust_score_components JSONB column to brokers table if it doesn't exist"""
    try:
        # Check if column exists
        result = supabase.rpc('check_column_exists', {
            'table_name': 'brokers',
            'column_name': 'trust_score_components'
        }).execute()
        
        if not result.data:
            print("✓ trust_score_components column already exists")
        else:
            print("✓ trust_score_components column confirmed")
            
    except Exception as e:
        print(f"Note: Column check function not available, assuming column exists: {e}")
...
def update_broker_trust_scores():
    """Update trust scores for all brokers"""
    try:
        # Get all brokers
        result = supabase.table('brokers').select('*').execute()
        brokers = result.data
        ...
        for broker in brokers:
            try:
                ...
                trust_score_components = calculate_broker_trust_score(broker)
                
                # Update broker in database
                update_result = supabase.table('brokers').update({
                    'trust_score': trust_score_components['overall'],
                ...
                if update_result.data:
                    updated_count += 1
                    print(f"✓ Updated trust score for {broker['name']}: {trust_score_components['overall']}")
                else:
                    error_count += 1
                    print(f"✗ Failed to update {broker['name']}")
...
def main():
    """Main function"""
    print("Broker Analysis Trust Score Update")
    print("=" * 40)
    
    # Step 1: Add trust_score_components column if needed
    print("\n1. Checking database schema...")
    if not add_trust_score_components_column():
        print("Failed to update database schema")
        return
    
    # Step 2: Update trust scores for all brokers
    print("\n2. Calculating and updating trust scores...")
    update_broker_trust_scores()
    
    print("\n✓ Trust score update process completed!")
